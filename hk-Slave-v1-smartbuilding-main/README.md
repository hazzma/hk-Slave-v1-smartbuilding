...
| IR Projector | Epson projector via IR | GPIO 3 dan GPIO 4 |
...
- Khusus pin input presence menggunakan pull-down internal (`INPUT_PULLDOWN`) yang diset secara dinamis saat modul diinisialisasi (bukan saat boot up).
...
- AC 1 output: GPIO 0.
- AC 2 output: GPIO 1.
- Projector output: GPIO 3 dan GPIO 4 (Semua jenis projector EPSON via Epson IR protocol 32-bit).
...
- Dukungan Epson projector IR protocol (V1.2).
- Penanganan error I2C recovery mid-run untuk BH1750 dan SCD30 (V1.2).
- Inisialisasi dinamis internal pull-down presence (V1.2).
...
- Panasonic AC IR payload dan Epson projector IR code masih perlu divalidasi dengan device asli.
