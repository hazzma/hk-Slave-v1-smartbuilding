...
bool IRDriver::sendEpsonProjector(const uint64_t data, uint16_t bits, uint16_t repeat) {
    if (!_initialized) {
        return false;
    }
    
    // Transmit sequentially on both projector outputs A and B using Epson protocol
    _irSendProjA.sendEpson(data, bits, repeat);
    _irSendProjB.sendEpson(data, bits, repeat);
    return true;
}
