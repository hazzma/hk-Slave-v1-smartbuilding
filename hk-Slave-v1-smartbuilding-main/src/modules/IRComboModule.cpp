...
void IRComboModule::executeCommand() {
    ...
    if (_activeCmd.target == IR_TARGET_AC1 || _activeCmd.target == IR_TARGET_AC2) {
        ...
        // Epson Projector standard 32-bit discrete codes
        // 0xC1AA30CFULL is Power On, 0xC1AA20DFULL is Power Off
        uint64_t data = _activeCmd.power ? 0xC1AA30CFULL : 0xC1AA20DFULL; 
        
        success = _driver->sendEpsonProjector(data, 32);
